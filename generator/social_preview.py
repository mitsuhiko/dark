...
class SocialPreviewGenerator:
    ...
    def __init__(self, project_folder):
        ...
        self.background_color = "#111"  # Dark background
        self.text_color = "#ffffff"  # White text
        self.header_color = "#f0e4d4"  # Header color (cream)
        self.author_color = "#c4b49a"  # Faded color for author (muted cream)

        self.fonts = self._load_fonts()
        self.avatar = self._load_avatar_optional()
        self.cache_file = (
    ...
    def _load_fonts(self):
        ...
        playfair_600_path = fonts_dir / "playfair-display-v40-latin-600.woff2"
        fonts["title"] = ImageFont.truetype(str(playfair_600_path), 64)
        fonts["title_medium"] = ImageFont.truetype(str(playfair_600_path), 56)
        fonts["title_small"] = ImageFont.truetype(str(playfair_600_path), 48)

        crimson_path = fonts_dir / "crimson-text-v19-latin-regular.woff2"
        fonts["body"] = ImageFont.truetype(str(crimson_path), 38)
        fonts["body_small"] = ImageFont.truetype(str(crimson_path), 34)
        fonts["author"] = ImageFont.truetype(str(crimson_path), 28)
